...
function applyExtends(config, filePath) {
    ...
    config = configExtends.reduceRight(function(previousValue, parentPath) {
        ...
        try {
            ...
            var value = load(parentPath);
            Object.keys(value.rules).forEach(function(rule) {
                var redundant = previousValue.rules && previousValue.rules[rule] &&
                    value.rules && value.rules[rule] &&
                    stringify(value.rules[rule], {cmp: sortByKey}) === stringify(previousValue.rules[rule], {cmp: sortByKey});
                if (redundant) {
                    debug("redundant rule definition: remove %s from %s", rule, filePath);
                }
            });
            return ConfigOps.merge(value, previousValue);
        } catch (e) {
