...
function getSourceCodeOfFile(filename, options) {
    ...
    if (results && results.results[0] && results.results[0].messages[0] && results.results[0].messages[0].fatal) {
        return [null, results.results[0].messages[0].message];
    }
...
function getSourceCodeOfFiles(patterns, options, cb) {
    ...
    filenames.forEach(function(filename) {
        var scResults = getSourceCodeOfFile(filename, opts);
        var sourceCode = scResults[0];
        var fatalError = scResults[1];
        if (sourceCode) {
            debug("got sourceCode of", filename);
            sourceCodes[filename] = sourceCode;
        }
        if (fatalError) {
            debug("failed to get sourceCode of", filename);
            log.error(filename + ": " + fatalError);
        }
        if (cb) {
