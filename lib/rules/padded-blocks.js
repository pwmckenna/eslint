...
module.exports = function(context) {
    ...
    function getCommentsInNode(node) {
        var allComments = context.getAllComments(),
            ...
            startIndex = 0,
            endIndex = allComments.length - 1,
            comment;

        while ((comment = allComments[startIndex]) && comment.loc.start.line <= start) {
            startIndex++;
        }

        while ((comment = allComments[endIndex]) && end <= comment.loc.end.line) {
            endIndex--;
        }

        return allComments.slice(startIndex, endIndex + 1);
    }
